  ...
  async createUser(formData: any): Promise<boolean> {
    return new Promise((resolve, reject) => {
      this.http.post<{id: string, message: string}>(`${environment.apiUrl}/user`, formData, { headers: { 'Content-Type': 'application/json' } })
          .subscribe({
            next: (response) => {
              this.userSubject.next({_id: response.id, user_name: formData.username, email: formData.email, friendNameList: [], friendRequestsNumber: 0});
              resolve(true);
          }, error: (error) => {
            resolve(false);
          }
  ...
  async logoutUser(): Promise<{ success: boolean, message: string }> {
    ...
    return new Promise((resolve, reject) => {
      this.http.post<{ message: string }>(`${environment.apiUrl}/auth/logout`, {}, {
        ...
        next: (response) => {
          this.userSubject.next({ _id: '', user_name: '', email: '', friendNameList: [], friendRequestsNumber: 0 });
  ...
  async getUserFromDb(): Promise<{success: boolean, user: any}> {
    return new Promise((resolve, reject) => {
      this.http.get<{createdAt: string, id: string, email: string, username: string}>(`${environment.apiUrl}/user`, {
        ...
        next: async (response) => {
          let friends = await this.getFriends() as Friend[];
          let {isSuccess: isSucces, data: friendRequests} = await this.getIncomingFriendRequests();
          const user = {
            _id: response.body?.id || '',
            user_name: response.body?.username || '',
            email: response.body?.email || '',
            friendNameList: friends,
  ...
  async getIncomingFriendRequests(): Promise<{isSuccess: boolean, data: any[]}> {
    return new Promise((resolve, reject) => {
      this.http.get<{users: []}>(`${environment.apiUrl}/friends/requests/ingoing`, {
        ...
        next: (response) => {
          this.userSubject.next({ ...this.userSubject.value, friendRequestsNumber: response.body?.users.length || 0 });
          resolve({isSuccess: true, data: response.body?.users || []});
        }, error: (error) => {
    ...
    return new Promise((resolve, reject) => {
      this.http.get<{users: []}>(`${environment.apiUrl}/friends`, {
        ...
        next: (response) => {
          ...
          this.userSubject.next({ ...this.userSubject.value, friendNameList: response.body?.users || [] });
          resolve({isSuccess: true, data: response.body?.users || []});
        }, error: (error) => {
          console.error('Failed to get friends:', error);
          reject({isSuccess: false, data: []});
  ...
  async acceptFriendRequest(id: string): Promise<boolean> {
    return new Promise((resolve, reject) => {
      this.http.put<{ message: string }>(`${environment.apiUrl}/friends/request/${id}/accept`, {}, {
        ...
        next: (response) => {
          this.getFriendList();
          this.userSubject.next({ ...this.userSubject.value, friendRequestsNumber: this.userSubject.value.friendRequestsNumber - 1 });
          resolve(true);
        }, error: (error) => {
    ...
    return new Promise((resolve, reject) => {
      this.http.delete<{ message: string }>(`${environment.apiUrl}/friends/${id}`, {
        ...
        withCredentials: true
          ...
          next: (response) => {
            this.userSubject.next({ ...this.userSubject.value, friendNameList: this.userSubject.value.friendNameList.filter((friend) => friend.id !== id) });
            resolve(true);
          }, error: (error) => {
