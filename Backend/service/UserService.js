 ...
 * Lists all users whose username partially matches the search term
 *
 * username String Partial or complete username to search for
 * returns UserList
 **/
exports.userSearch = function(username, userId) {
  return new Promise(async function(resolve, reject) {
    try {
      const users = await User.find({ username: { $regex: username, $options: 'i' }, isConfirmed: true, _id: { $ne: userId } }).sort({ username: 1 });
